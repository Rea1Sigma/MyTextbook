...
{				
	...
	if (btsEncKey)
	{
		LONG LenKey = strlen(btsEncKey);
		for (LONG i = 0, j = 0; i < iBytesRead; i++)
		{
			pBuff[i] = pBuff[i] ^ btsEncKey[j];
			j++;
			if (j >= LenKey)
				j = 0;
		}
			  ...
			  char * btsEncKey /*=NULL*/ )
{
	...
	LONG ret = EFPutBytes(hFile, llWritePos, buff, iLenBytes-1, iShowResume, szFailInfo, btsEncKey);
	delete []buff;
	return ret;
}

LONG EFPrint( HANDLE hFile, tstring stringText, eEFLineFeed styleLineFeed /*= EF_LineSeed_CrLf*/, LONGLONG llWritePos /*= -1*/, int iShowResume /*= 1*/, tstring stringFailInfo /*= TEXT("无 ㄏ蛭募行慈胱??)*/ , char* btsEncKey /*=NULL*/)
{
	return EFPrint(hFile, stringText.c_str(), styleLineFeed, llWritePos, iShowResume, stringFailInfo.c_str(), btsEncKey);
}
