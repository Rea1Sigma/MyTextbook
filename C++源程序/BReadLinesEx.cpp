...
int CBReadLinesEx::GetNextLine( LPTSTR &szLine )
{
    ...
    while ( m_buff.llStartPosAbso <= m_llFileLength-1 )
	{
        ...
        if ( m_buff.iPtrInBuf < 0 ) 
		{
            ...
            m_buff.iBufLen = EFGetBytes(m_hFile, m_buff.llStartPosAbso, m_buff.bufBytes, mc_RL_BufLen, 0,TEXT("无法读取文件"),EncryptKey);
        ...
        if (m_buff.iPtrInBuf < m_buff.iBufLen)    // 是否在 .bufBytes[] 中找到了 \r或\n
        {
            ...
            else
			{
					...
					m_buff.iIgnoreFirstLf > 0)
				{
                    ...
                    tRet = EFGetBytes(m_hFile, m_buff.llStartPosAbso + m_buff.iBufLen, tByt, mc_RL_BufLen, 0, TEXT("无法读取文件"), EncryptKey);
